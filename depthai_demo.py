...
class DepthAI:
    ...
    def startLoop(self):
        ...
        while self.runThread:
            ...
            for packet in self.data_packets:
                ...
                if packetData is None:
                    ...
                    cv2.imwrite('plate.png', mat)
                    print("Looking for license plate")
                    os.system("alpr -j plate.png > json")
                    with open("json") as j:
                        data = json.load(j)
                        if data["results"]:
                            print("Found plate: " + data["results"][0]["plate"])
        ...
        del p  # in order to stop the pipeline object should be deleted, otherwise device will continue working. This is required if you are go ing to add code after the main loop, otherwise you can ommit it.
        del self.device
